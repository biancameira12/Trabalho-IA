...
'''
    #TENTATIVA 2
    position, foodGrid = state
    lstFood = foodGrid.asList()
    cost = 0
    for food in lstFood:
        xy1 = position
        xy2 = food
        costFood = abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
        if(costFood > cost):
            cost = costFood

    return 2 * foodGrid.count() + cost
'''
...
'''
    ...
    position, foodGrid = state
    cost = 0
    if(not problem.heuristicInfo.has_key('listVisited')):
        list = []
        problem.heuristicInfo['listVisited'] = list

    if(position in problem.heuristicInfo['listVisited']):
        cost = len(problem.heuristicInfo['listVisited'])

    problem.heuristicInfo['listVisited'].append(position)
